...
class CarGraphicsItem(QGraphicsItemGroup):
    ...
    Car graphics item composed of:
      ...
      - ID label (ignores view transforms for constant size).
    ...
    def __init__(
        ...
        # Scale so that rendered width matches workspace car_width (maintains aspect ratio)
        pm_w = max(1, car_pixmap.width())
        pm_h = max(1, car_pixmap.height())
        img_scale = self._car_width / float(pm_w)
        ...
        if show_collision_box:
            ...
            pen = QPen(QColor(30, 60, 90), 2)
            pen.setCosmetic(True)  # Constant pixel width regardless of scale
            self._collision_rect_item.setPen(pen)
            self._collision_rect_item.setBrush(QBrush(Qt.GlobalColor.transparent))
            self.addToGroup(self._collision_rect_item)

        # -------------------------
        # 3) ID label (scale-independent)
        # -------------------------
        self._label = QGraphicsSimpleTextItem(str(car_id))
        self._label.setBrush(QBrush(Qt.GlobalColor.white))

        # Make label ignore all transforms (view scale, rotation, Y-flip)
        # This ensures text is always readable at constant size
        ...
        # Position at car center (will be adjusted by ItemIgnoresTransformations)
        self._label.setPos(0, 0)
        # ---- ADD THESE ----
        self._label.setAcceptedMouseButtons(Qt.MouseButton.NoButton)
        self._label.setFlag(QGraphicsItem.GraphicsItemFlag.ItemIsSelectable, False)


        self.addToGroup(self._label)

        print("ws car_w/h:", self._car_width, self._car_height,
      "offset:", self._offset_w, self._offset_h)
        print("pix pm_w/h:", pm_w, pm_h, "img_scale:", img_scale,
            "car_w/img_scale:", self._car_width/img_scale)
...
class MVPCanvas(QGraphicsView):
    ...
    def _show_click_point(self, x: float, y: float, radius_px: float = 3.0) -> None:
        """
        Show/update a small point at (x,y) in workspace coordinates.

        Args:
            x, y: Position in workspace coordinates
            radius_px: Desired radius in screen pixels (will be converted to workspace units)
        """
        # Convert pixel radius to workspace units
        r = radius_px / self._scale if self._scale > 0 else radius_px

        if self._click_point_item is None:
            self._click_point_item = QGraphicsEllipseItem()
            pen = QPen(QColor(255, 80, 80), 2)
            pen.setCosmetic(True)  # Constant pixel width regardless of scale
            self._click_point_item.setPen(pen)
    ...
    def _update_drawing_item(self, uuid: str, drawing: Dict[str, Any]) -> None:
        ...
        if draw_type == "line" and isinstance(item, QGraphicsLineItem):
            ...
            x, y = drawing.get("position", (0, 0))
            radius_px = drawing.get("radius", 4)
            # Convert pixel radius to workspace units
            r = radius_px / self._scale if self._scale > 0 else radius_px
    ...
    def _create_point(self, drawing: Dict[str, Any]) -> QGraphicsEllipseItem:
        ...
        The radius is interpreted as pixels and converted to workspace units
        for consistent UI appearance regardless of workspace scale.
        """
        x, y = drawing.get("position", (0, 0))
        radius_px = drawing.get("radius", 4)
        # Convert pixel radius to workspace units
    ...
    def _apply_pen(self, item: QGraphicsItem, drawing: Dict[str, Any]) -> None:
        ...
        Pens are cosmetic (constant pixel width) by default for consistent UI appearance.
        """
    ...
    def _apply_pen_brush(self, item: QGraphicsItem, drawing: Dict[str, Any]) -> None:
        ...
        # Only apply fill if explicitly specified, otherwise transparent (outline only)
        if "fill" in drawing:
